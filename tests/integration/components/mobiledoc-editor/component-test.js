...
import { setupRenderingTest, render as renderSync } from 'ember-qunit';
import { module, test } from 'qunit';
import { click, fillIn, settled, render, pauseTest } from '@ember/test-helpers';
import {
  selectRange,
...
module('Integration | Component | mobiledoc editorc', function(hooks) {
...
//moduleForComponent('mobiledoc-editor', 'Integration | Component | mobiledoc editor', {
  ...
  hooks.beforeEach(function() {
    ...
    this.registerCardComponentWithEditor = (cardName, template, componentClass, editorTemplate, editorClass=Component.extend()) => {
      ...
      this.owner.register(`template:components/${cardName}-editor`, editorTemplate);
      return card;
    };
  });
  ...
  test('it boots the mobiledoc editor', function(assert) {
    assert.expect(2);
    return render(hbs`{{mobiledoc-editor}}`).then(()=>{
      let editors = this.element.querySelectorAll('.mobiledoc-editor__editor');
      assert.ok(editors.length, 'Mobiledoc editor exists');
      assert.ok(editors[0].contentEditable, 'Mobiledoc editor is booted');
    });
  });
  ...
  test('it boots mobiledoc editor with mobiledoc', function(assert) {
    ...
    return render(hbs`{{mobiledoc-editor mobiledoc=mobiledoc}}`).then(()=>{
      let editors = this.element.querySelectorAll('.mobiledoc-editor__editor');
      assert.ok(editors[0].contentEditable, 'Mobiledoc editor is booted');
      assert.ok(editors[0].textContent.includes("Howdy"), 'Mobiledoc editor is booted with text');
    })
  });
  ...
  test('it does not create a new editor when the same mobiledoc is set', function(assert) {
    ...
    this.set('mobiledoc', mobiledoc);
    this.set('willCreateEditor', () => willCreateCalls++);
    this.set('didCreateEditor', (_editor) => {
      editor = _editor;
      didCreateCalls++;
    });
    return render(hbs`
  ...
  test('wraps component-card adding in runloop correctly', function(assert) {
    ...
    this.set('mobiledoc', mobiledoc);
    this.set('didCreateEditor', (_editor) => editor = _editor);
    ...
    renderSync(hbs`
      {{#mobiledoc-editor did-create-editor=didCreateEditor mobiledoc=mobiledoc cards=cards as |editor|}}
      {{/mobiledoc-editor}}
    `);
  ...
  test('it updates the editor when the mobiledoc changes', function(assert) {
    ...
    return render(hbs`{{mobiledoc-editor mobiledoc=mobiledoc}}`).then(() => {
      assert.ok(
        !!this.element.querySelectorAll('.mobiledoc-editor__editor')[0].textContent.includes("Howdy"),
      ...
      assert.ok(
        !!this.element.querySelectorAll('.mobiledoc-editor__editor')[0].textContent.includes("Doody"),
  ...
  test('it renders a yielded toolbar', function(assert) {
    ...
    `).then(() => {
      assert.ok(
        this.element.querySelectorAll('.mobiledoc-editor')[0].textContent.includes("Toolbar"),
        'Toolbar is yielded'
      );
    });
  });
  ...
  test('it displays a custom placeholder', function(assert) {
    ...
    `).then(() => {
      assert.equal(
        this.element.querySelectorAll('[data-placeholder]')[0].getAttribute("data-placeholder"),
  ...
  test('passes through spellcheck option', function(assert) {
    ...
    `).then(() => {
      assert.equal(
        this.element.querySelectorAll('[spellcheck]')[0].getAttribute('spellcheck'),
        "false"
      );
    });
  });
  ...
  test('it bolds the text and fires `on-change`', function(assert) {
    ...
    let changeEvents = [];
    this.set('myOnChange', (mobiledoc) => {
      changeEvents.push(mobiledoc);
    });

    return render(hbs`
      {{#mobiledoc-editor mobiledoc=mobiledoc on-change=myOnChange as |editor|}}
        <button {{action editor.toggleMarkup 'strong'}}>Bold</button>
      {{/mobiledoc-editor}}
    `).then(() => {
      let textNode = this.element.querySelectorAll('p')[0].firstChild;
      assert.ok(textNode.textContent.includes(text));

      return selectRange(textNode, 0, textNode, text.length).then(() => {
        return click(this.element.querySelector('button')).then(() => {
          assert.ok(
            !!this.element.querySelectorAll('strong')[0].textContent.includes("Howdy"),
  ...
  test('serializes mobiledoc to `mobiledocVersion`', function(assert) {
    ...
    return render(hbs`
      {{#mobiledoc-editor mobiledoc=mobiledoc serializeVersion=serializeVersion on-change=(action myOnChange) as |editor|}}
        <button {{action editor.toggleMarkup 'strong'}}>Bold</button>
      {{/mobiledoc-editor}}
    `).then(() => {
      let textNode = document.evaluate(`.//p[contains(.,'${text}')]`, this.element, null, XPathResult.ANY_TYPE, null).iterateNext().firstChild;
      return selectRange(textNode, 0, textNode, text.length).then(() => {
        return click(this.element.querySelector('button')).then(() => {
          ...
          textNode = document.evaluate(`.//p//strong[contains(.,'${text}')]`, this.element, null, XPathResult.ANY_TYPE, null).iterateNext().firstChild;
  ...
  test('it exposes "toggleSection" which toggles the section type and fires `on-change`', function(assert) {
    ...
    let text = 'Howdy';
    this.set('mobiledoc', simpleMobileDoc(text));
    this.set('myOnChange', () => onChangeCount++);
    return render(hbs`
      {{#mobiledoc-editor mobiledoc=mobiledoc on-change=(action myOnChange) as |editor|}}
        <button {{action editor.toggleSection 'h2'}}>H2</button>
      {{/mobiledoc-editor}}
    `).then(() => {
      const textNode = document.evaluate(`.//p[contains(.,'${text}')]`, this.element, null, XPathResult.ANY_TYPE, null).iterateNext().firstChild;

      return selectRange(textNode, 0, textNode, text.length).then(() => {
        assert.ok(!this.element.querySelectorAll('h2').length, 'precond - no h2');
        assert.equal(onChangeCount, 0, 'precond - no on-change');

        return click(this.element.querySelector('button')).then(() => {
          assert.equal(onChangeCount, 1, 'fires on-change');
          assert.ok(!!this.element.querySelectorAll('h2:contains(Howdy)').length, 'Changes to h2 tag');

          onChangeCount = 0;
          return click(this.element.querySelector('button')).then(() => {
            assert.equal(onChangeCount, 1, 'fires on-change again');
            assert.ok(!this.element.querySelectorAll('h2').length, 'toggles h2 tag off again');
          });
        });
      });
    });
  ...
  test('it exposes "setAttribute" which can be used by section-attribute-button to update the section attribute value and fires `on-change`', function(assert) {
    ...
    return render(hbs`
      ...
      {{/mobiledoc-editor}}
    `).then(() => {
      const paragraphNode = document.evaluate(`.//p[contains(.,'${text}')]`, this.element, null, XPathResult.ANY_TYPE, null).iterateNext();
      const textNode = paragraphNode.firstChild;

      return selectRange(textNode, 0, textNode, text.length).then(() => {
        assert.ok(!this.element.querySelectorAll(`p:contains(${text})`).attr('data-md-text-align'), 'precond - no attr');
        assert.equal(onChangeCount, 0, 'precond - no on-change');
        let buttons = this.element.querySelectorAll('button');
        return click(buttons[buttons.length-1]);
      }).then(() => {
        assert.equal(onChangeCount, 1, 'fires on-change');
        assert.equal(this.element.querySelectorAll(`p:contains(${text})`).attr('data-md-text-align'), 'center', 'sets attribute');
        onChangeCount = 0;
        let buttons = this.element.querySelectorAll('button');
        return click(buttons[buttons.length-1]);
      }).then(() => {
        assert.equal(onChangeCount, 0, 'fires on-change again');
        assert.equal(this.element.querySelectorAll(`p:contains(${text})`).attr('data-md-text-align'), 'center', 'clicking again does nothing');
        onChangeCount = 0;
        let buttons = this.element.querySelectorAll('button');
        return click(buttons[0]);
      }).then(() => {
        assert.equal(onChangeCount, 1, 'fires on-change again');
        assert.equal(this.element.querySelectorAll(`p:contains(${text})`).attr('data-md-text-align'), undefined, 'clicking left removes attribute since it is the default value');
      });
    });
  });
  ...
  test('toolbar buttons can be active', function(assert) {
    ...
    `).then(() => {
      ...
      const button = this.element.querySelectorAll(`button[title=Heading]`)[0];
      return selectRange(textNode, 0, textNode, text.length).then(() => {
        assert.ok(button, 'has heading toolbar button');
        return click(button);
      }).then(() => {
        assert.ok(this.element.querySelectorAll(`h1:contains(${text})`).length, 'heading-ifies text');
        assert.ok(button.hasClass('active'), 'heading button is active');
        return click(button);
      }).then(() => {
        assert.ok(!this.element.querySelectorAll(`h1`).length, 'heading is gone');
  ...
  test('toolbar has list insertion button', function(assert) {
    ...
    `).then(() => {
      const textNode = document.evaluate(`.//p[contains(.,'${text}')]`, this.element, null, XPathResult.ANY_TYPE, null).iterateNext().firstChild;
      return selectRange(textNode, 0, textNode, text.length).then(() => {
        const button = this.element.querySelectorAll(`button[title=List]`);
        assert.ok(button.length, 'has list toolbar button');
        return click(button).then(() => {
          assert.ok(this.element.querySelectorAll(`ul li:contains(${text})`).length, 'list-ifies text');
        });
      });
    });
  });
  ...
  test('toggleLink action is no-op when nothing is selected', function(assert) {
    assert.expect(2);
    this.set('mobiledoc', simpleMobileDoc(''));
    this.set('didCreateEditor', (editor) => this._editor = editor);
    return render(hbs`
      {{#mobiledoc-editor autofocus=false mobiledoc=mobiledoc did-create-editor=didCreateEditor as |editor|}}
        <button {{action editor.toggleLink}}>Link</button>
      {{/mobiledoc-editor}}
    `).then(() => {
      assert.ok(!this._editor.hasCursor(), 'precond - no cursor');
      return click(this.element.querySelector('button')).then(() => {
        assert.ok(this.element.querySelectorAll('input').length === 0, 'no link input shown');
      });
    });
  });
  ...
  test('it links selected text and fires `on-change`', function(assert) {
    ...
    let text = 'Howdy';
    this.set('mobiledoc', simpleMobileDoc(text));
    this.set('myOnChange', (mobiledoc) => this._mobiledoc = mobiledoc);
    this.set('didCreateEditor', (editor) => this._editor = editor);
    return render(hbs`
      {{#mobiledoc-editor mobiledoc=mobiledoc on-change=myOnChange did-create-editor=didCreateEditor as |editor|}}
        <button id="test-toggle-link" {{action editor.toggleLink}}>Link</button>
      {{/mobiledoc-editor}}
    `).then(() => {
      ...
      return selectRangeWithEditor(editor, new MobiledocKit.Range(editor.post.headPosition(), editor.post.tailPosition())).then(() => {
        return click(this.element.querySelector('button#test-toggle-link')).then(() => nextFrame());
      }).then(() => {
        return fillIn(this.element.querySelector('input', 'http://example.com')).then(() => nextFrame())
      }).then(() => {
        return click(this.element.querySelector('button#test-toggle-link')).then(() => {
          let aElems = this.element.querySelectorAll('a[href="http://example.com"]')
          assert.ok(aElems.length);
          assert.ok(aElems[0].textContent.includes("Howdy"),'a tag contains text');
  ...
  test('it de-links selected text and fires `on-change`', function(assert) {
    ...
    this.set('myOnChange', (mobiledoc) => this._mobiledoc = mobiledoc);
    this.set('myDidCreateEditor', (editor) => this._editor = editor);
    return render(hbs`
      {{#mobiledoc-editor mobiledoc=mobiledoc on-change=myOnChange did-create-editor=myDidCreateEditor as |editor|}}
        <button {{action editor.toggleLink}}>Link</button>
      {{/mobiledoc-editor}}
    `).then(() => {
      let { _editor: editor } = this;
  ...
  test('it adds a component in display mode to the mobiledoc editor', function(assert) {
    ...
    return render(hbs`
      ...
      {{/mobiledoc-editor}}
    `).then(() => {
      return click(this.element.querySelector('button#add-card')).then(() => {
        assert.ok(this.element.querySelectorAll(`#demo-card`).length, 'Card added in display mode');
        assert.ok(!this.element.querySelectorAll(`#demo-card-editor`).length, 'Card not in edit mode');
        assert.ok(this.element.querySelectorAll('button#edit-card').length, 'has edit card button');

        return click(this.element.querySelector('button#edit-card')).then(() => {
          assert.ok(!this.element.querySelectorAll(`#demo-card`).length, 'Card not in display mode');
          assert.ok(this.element.querySelectorAll(`#demo-card-editor`).length, 'Card changed to edit mode');
        });
      });
    });
  });
  ...
  test('it adds a card and removes an active blank section', function(assert) {
    ...
    this.set('didCreateEditor', (_editor) => editor = _editor);
    return render(hbs`
      {{#mobiledoc-editor mobiledoc=mobiledoc cards=cards did-create-editor=didCreateEditor as |editor|}}
        <button id='add-card' {{action editor.addCard 'demo-card'}}></button>
      {{/mobiledoc-editor}}
    `).then(() => {
      assert.equal(this.element.querySelectorAll('.mobiledoc-editor p').length, 1, 'blank section exists');
      assert.equal(this.element.querySelectorAll('#demo-card').length, 0, 'no card section exists');
      editor.selectRange(editor.post.headPosition());
      return click(this.element.querySelector('button#add-card')).then(() => {
        assert.equal(this.element.querySelectorAll('.mobiledoc-editor p').length, 0, 'no blank section');
        assert.equal(this.element.querySelectorAll('#demo-card').length, 1, 'card section exists');
      });
    });
  });
  ...
  test('it adds a card and focuses the cursor at the end of the card', function(assert) {
    ...
    this.set('mobiledoc', simpleMobileDoc());
    return render(hbs`
      {{#mobiledoc-editor autofocus=false did-create-editor=didCreateEditor mobiledoc=mobiledoc cards=cards as |editor|}}
        <button id='add-card' {{action editor.addCard 'demo-card'}}>ADD CARD</button>
      {{/mobiledoc-editor}}
    `).then(() => {
      ...
      return selectRangeWithEditor(editor, editor.post.tailPosition()).then(() => {
        ...
        let button = this.element.querySelectorAll('button#add-card')[0];
        return click(button);
      }).then(() => {
        assert.equal(this.element.querySelectorAll('#demo-card').length, 1, 'card section exists');

        let cardWrapper = this.element.querySelectorAll('#demo-card').parents('.__mobiledoc-card');
        ...
        assert.ok(document.activeElement === document.querySelectorAll('.__mobiledoc-editor')[0],
  ...
  test('can add a card to a blank post', function(assert) {
    ...
    return render(hbs`
      ...
      {{/mobiledoc-editor}}
    `).then(() => {
      let editorEl = this.element.querySelectorAll('.mobiledoc-editor__editor')[0];
      return selectRange(editorEl, 0, editorEl, 0).then(() => {
        assert.ok(editor.hasCursor(), 'precond - editor has cursor');
        assert.ok(!this.element.querySelectorAll('#demo-card').length, 'precond - no card inserted');
        run(() => editor.insertCard('demo-card'));
        return settled();
      }).then(() => {
        assert.ok(this.element.querySelectorAll('#demo-card').length, 'inserts card');
      });
    });
  });
  ...
  test('it has `addCardInEditMode` action to add card in edit mode', function(assert) {
    ...
    `).then(() => {
      return click(this.element.querySelector('button#add-card')).then(() => {
        assert.ok(!this.element.querySelectorAll(`#demo-card`).length, 'Card added in edit mode');
        assert.ok(this.element.querySelectorAll(`#demo-card-editor`).length, 'Card not in display mode');
      });
    });
  });
  ...
  test('(deprecated) `addCard` passes `data`, breaks reference to original payload', function(assert) {
    ...
    `).then(() => {
      return click(this.element.querySelector('button#add-card')).then(() => {
        ...
        assert.ok(this.element.querySelectorAll('button#mutate-payload').length,
                  'has mutate-payload button');
        assert.ok(this.element.querySelectorAll(`#demo-card:contains(${payload.foo})`).length,
                  'displays passed `data`');
        return click(this.element.querySelector('button#mutate-payload')).then(() => {
  ...
  test('#activeSectionTagNames is correct', function(assert) {
    ...
    return render(hbs`
      ...
      {{/mobiledoc-editor}}
    `).then(() => {
      let bq = document.evaluate(`.//blockquote[contains(.,'blockquote section')]`, this.element, null, XPathResult.ANY_TYPE, null).iterateNext();
      return moveCursorTo(this, bq).then(() => {
        assert.ok(this.element.querySelectorAll('#is-block-quote').length, 'is block quote');
        let pnode = document.evaluate(`.//p[contains(.,'first paragraph')]`, this.element, null, XPathResult.ANY_TYPE, null).iterateNext();
        return moveCursorTo(this, pnode);
      }).then(() => {
        assert.ok(this.element.querySelectorAll('#is-p').length, 'is p');
      });
    });
  });
  ...
  test('#activeSectionTagNames is correct when a card is selected', function(assert) {
    ...
    this.set('cards', [{
      ...
      render() {
        ...
        div.setAttribute('id', 'card-test');
        div.appendChild(document.createTextNode("CARD CONTENT"));
        return div;
      }
    }]);
    return render(hbs`
    ...
    `).then(() => {
      ...
      return moveCursorTo(this, '.mobiledoc-editor p').then(() => {
        assert.ok(el.querySelectorAll('#is-p').length, 'precond - is p');
        return moveCursorTo(this, '#card-test');
      }).then(() => {
        assert.ok(el.querySelectorAll('#not-p').length, 'is not p');
      });
    });
  });
  ...
  test('exposes `addAtom` action to add an atom', function(assert) {
    ...
    this.set('onChange', (_mobiledoc) => mobiledoc = _mobiledoc);
    this.set('didCreateEditor', (_editor) => editor = _editor);
    return render(hbs`
      {{#mobiledoc-editor on-change=onChange did-create-editor=didCreateEditor mobiledoc=mobiledoc atoms=atoms as |editor|}}
    ...
    `).then(() => {
      ...
      return selectRangeWithEditor(editor, new MobiledocKit.Range(editor.post.headPosition())).then(() => {
        let buttons = this.element.querySelectorAll('button#add-atom');
        assert.ok(buttons.length, 'precond - has button');
        assert.ok(!this.element.querySelectorAll('span:contains(I AM AN ATOM)').length, 'precond - no atom');
        return click(buttons[0]);
      }).then(() => {

        assert.ok(el.querySelectorAll('span:contains(I AM AN ATOM)').length, 'atom is added after clicking');
  ...
  test('wraps component-atom adding in runloop correctly', function(assert) {
    ...
    this.set('mobiledoc', simpleMobileDoc());
    renderSync(hbs`
      {{#mobiledoc-editor did-create-editor=didCreateEditor mobiledoc=mobiledoc atoms=atoms as |editor|}}
      {{/mobiledoc-editor}}
    `);
    assert.ok(!run.currentRunLoop, 'precond - no run loop');
    editor.run((postEditor) => {
    ...
    assert.ok(this.element.querySelectorAll('#demo-atom').length, 'demo atom is added');
  });
  ...
  test('does not rerender atoms when updating text in section', function(assert) {
    ...
    return render(hbs`
      {{#mobiledoc-editor mobiledoc=mobiledoc
                          atoms=atoms
                          did-create-editor=didCreateEditor as |editor|}}
      {{/mobiledoc-editor}}
    `).then(() => {
      renderCount = 0;
